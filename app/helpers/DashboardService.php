...
class DashboardService {
    ...
    public function logVisit($page) {
        ...
        try {
            $this->db->query("INSERT INTO activity_logs (user_id, action, module, target_id, ip_address) 
                            VALUES (:userId, :action, :module, NULL, :ipAddress)");
            $this->db->bind(':userId', $_SESSION['user_id'] ?? null);
            $this->db->bind(':action', 'visit');
            $this->db->bind(':module', $page);
            $this->db->bind(':ipAddress', $_SERVER['REMOTE_ADDR'] ?? null);
    ...
    /**
     * Log a visit in the admin_notifications table (for backwards compatibility)
     * 
     * @param string $page The page being visited
     * @return bool Success status
     */
    public function logVisitToAdminNotifications($page) {
        if (!$this->db) {
            $this->db = new Database();
        }
        ...
        try {
            ...
            $this->db->query("SHOW TABLES LIKE 'admin_notifications'");
            $tableExists = $this->db->resultSet();
            
            if (!empty($tableExists)) {
                $this->db->query("INSERT INTO admin_notifications (type, title, message, ip_address) 
                                VALUES ('visit', :page, 'Page visit', :ipAddress)");
            ...
            error_log('Error logging visit to admin_notifications: ' . $e->getMessage());
            return false;
        }
    }
}